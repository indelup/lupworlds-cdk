...
if (!tableName) throw new Error("MATERIALS_TABLE_NAME not set");

const bucketName = process.env.MATERIALS_IMAGES_BUCKET_NAME;
if (!bucketName) throw new Error("MATERIALS_IMAGES_BUCKET_NAME not set");
...
app.post("/", resolveWorldId, async (c) => {
    try {
        ...
        const newMaterial = { ...body, worldId, id: randomUUID(), createdAt: new Date().toISOString() };
        await ddbDocClient.send(new PutCommand({ TableName: tableName, Item: newMaterial }));
...
app.put("/:id", requireItemWorldWrite(tableName), async (c) => {
    ...
    try {
        ...
        if (existing.characterSrc && existing.characterSrc !== updatedMaterial.characterSrc) {
            try {
                await s3Client.send(
        ...
        if (existing.backgroundSrc && existing.backgroundSrc !== updatedMaterial.backgroundSrc) {
            try {
                await s3Client.send(
        ...
        await ddbDocClient.send(
            new PutCommand({ TableName: tableName, Item: { ...updatedMaterial, id: materialId } }),
        );
        return c.json({ message: "Material updated successfully", material: { ...updatedMaterial, id: materialId } });
    } catch (error: any) {
        console.error(error);
        return c.json({ error: error.message }, 500);
    }
...
app.delete("/:id", requireItemWorldWrite(tableName), async (c) => {
    ...
    try {
        ...
        await ddbDocClient.send(new DeleteCommand({ TableName: tableName, Key: { id: materialId } }));
        return c.json({ message: "Material deleted successfully", deletedMaterial: existing });
    } catch (error: any) {
        console.error(error);
        return c.json({ error: error.message }, 500);
    }
